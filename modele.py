...
class Projectile:
    def __init__(self, x, y, typeBullet):
        ...
        self.taille_y = 10
        self.vitesse = -10
        self.goodBad = typeBullet # "g": ally bullet, "b": enemy bullet

        if self.goodBad == "b":
            self.vitesse = 10
        
    def typeProjectile(self):
        self.vitesse = -10
        if self.goodBad == "b":
            self.vitesse *= -1
...
class Modele:
    ...
    def mise_a_jour(self):
        ...
        for p in self.vaisseau.projectiles:
            if p.goodBad == "g":
                for o in self.ovnis:
                    if p.x <= o.x + o.taille_x / 2 and p.x >= o.x - o.taille_x / 2:
        ...
        # Nettoyage des objets sortis de l'écran (À COMPLETER)
        self.ovnis = [
